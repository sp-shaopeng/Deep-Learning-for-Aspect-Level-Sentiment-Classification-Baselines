...
data_dir='../data/data_orign/SemEval2014-Task4/'
dest_dir='../data/data_processed/SemEval2014/'

dir_path = dest_dir+'bert-pair/restaurants14/'
if not os.path.exists(dir_path):
    os.makedirs(dir_path)

with open(dir_path+"test_NLI_M.csv","w",encoding="utf-8") as g:
    with open(data_dir+"test/Restaurants_Test.xml","r",encoding="utf-8") as f:
        s=f.readline().strip()
        while s:
            category=[]
            polarity=[]
...
with open(dir_path+"train_NLI_M.csv","w",encoding="utf-8") as g:
    with open(data_dir+"train/Restaurants_Train_v2.xml","r",encoding="utf-8") as f:
        s=f.readline().strip()
        while s:
            category=[]
            polarity=[]
